...
import { CustomColorDef, CustomListDef } from "./preamble/LanguageDb";
...
import {
  CodeHighlightingTab,
  CodeHighlightConfig,
} from "./preamble/tabs/CodeHighlightingTab";
...
export const PreambleWizard: React.FC<PreambleWizardProps> = ({ onInsert }) => {
  ...
  const [config, setConfig] = useState<PreambleConfig>({
    ...
    pkgHyperref: true,
    hyperrefColors: true,
    hyperrefLinkColor: "blue",
    hyperrefUrlColor: "blue",
    pkgTikz: false,
    pkgPgfplots: false,
    pkgBooktabs: false,
    pkgFloat: false,
  ...
  const handleCodeColorChange = (
    key: keyof CodeHighlightConfig["lstColors"],
    val: string
  ) => {
  ...
  const handleAddColor = (
    ...
    setCustomColors((prev) => [
      ...prev,
      { id: Date.now(), name, model, value, previewHex },
    ]);
  };
  ...
  const handleAddList = (
    ...
    setCustomLists((prev) => [
      ...prev,
      { id: Date.now(), name, baseType: type, options },
    ]);
  };

  // --- Code Generation Effect ---
  useEffect(() => {
    const code = generateFullPreamble(
      config,
      customColors,
      customLists,
      codeSettings
      ...
      <Grid gutter={0} style={{ flex: 1, overflow: "hidden" }}>
        ...
        >
          ...
          <ScrollArea style={{ flex: 1 }} p="md" type="hover">
            {activeTab === "general" && (
              <GeneralTab config={config} onChange={handleChange} />
            )}
            {activeTab === "layout" && (
              <LayoutTab config={config} onChange={handleChange} />
            )}
            {activeTab === "packages" && (
              <PackagesTab config={config} onChange={handleChange} />
            )}
            {activeTab === "biblio" && (
              <BibliographyTab config={config} onChange={handleChange} />
            )}
            {activeTab === "code" && (
              <CodeHighlightingTab
                config={codeSettings}
            ...
            {activeTab === "lists" && (
              <ListsTab
                ...
                onRemoveList={(id) =>
                  setCustomLists((l) => l.filter((x) => x.id !== id))
                }
              />
            )}
            {activeTab === "colors" && (
              <ColorsTab
                ...
                onRemoveColor={(id) =>
                  setCustomColors((c) => c.filter((x) => x.id !== id))
                }
              />
            )}
          </ScrollArea>
