...
import { PAGE_WIDTH_PX, PAGE_HEIGHT_PX, CM_TO_PX } from "../LanguageDb";
...
export const PageVisualizer: React.FC<PageVisualizerProps> = ({ config }) => {
  const styles = useMemo(() => {
    ...
    const headerHeightPx = config.pkgGeometry
      ? config.headHeight * CM_TO_PX
      : 0;
        ...
        >
          {config.columns === "two" ? (
            ...
            >
              ...
              <div style={{ fontSize: 8, color: "#999", marginTop: 5 }}>
                {(styles.bodyWidthPx / CM_TO_PX).toFixed(1)}cm width
              </div>
            </div>
          )}
        </div>
