...
export const ColorsTab: React.FC<ColorsTabProps> = ({
  ...
  const selectedOptions = (config as any).xcolorOptions || [
    "table",
    "dvipsnames",
  ];
  ...
  const [snipType, setSnipType] = useState<
    "textcolor" | "colorbox" | "fcolorbox" | "pagecolor" | "rowcolor"
  >("textcolor");
  ...
  const handleInsertSnippet = () => {
    ...
    switch (snipType) {
      case "textcolor":
        code = `\\textcolor{${snipColor}}{${snipText}}`;
        break;
      case "colorbox":
        code = `\\colorbox{${snipColor}}{${snipText}}`;
        break;
      case "fcolorbox":
        code = `\\fcolorbox{${snipSecColor}}{${snipColor}}{${snipText}}`;
        break;
      case "pagecolor":
        code = `\\pagecolor{${snipColor}}`;
        break;
      case "rowcolor":
        code = `\\rowcolor{${snipColor}}`;
        break;
    }
    onInsert(code);
  };
  ...
  const allColors = useMemo(() => {
    return Array.from(
      new Set([...STANDARD_LATEX_COLORS, ...customColors.map((c) => c.name)])
    );
  }, [customColors]);
          ...
          <Box p="md">
              ...
              <Group align="flex-start" grow>
                <Stack gap="xs" style={{ flex: 1 }}>
                  ...
                  <Select
                    ...
                    data={COLOR_MODELS}
                    value={palModel}
                    onChange={(v) => setPalModel(v || "HTML")}
                    searchable
                  />
                  <TextInput
                    label={`Calculated Value (${palModel})`}
                    value={palCalculated}
                    readOnly
                    variant="filled"
                    rightSection={
                      <Tooltip label="Auto-converted">
                ...
                <SimpleGrid cols={2} spacing="xs">
                  <Group gap="xs" align="center">
                    <NumberInput
                      ...
                      value={cmykValues.c}
                      onChange={(v) =>
                        setCmykValues({ ...cmykValues, c: Number(v) })
                      }
                    />
                    <Slider
                      style={{ flex: 1 }}
                      color="cyan"
                      value={cmykValues.c}
                      onChange={(v) => setCmykValues({ ...cmykValues, c: v })}
                      label={null}
                    />
                  </Group>
                  <Group gap="xs" align="center">
                    <NumberInput
                      ...
                      value={cmykValues.m}
                      onChange={(v) =>
                        setCmykValues({ ...cmykValues, m: Number(v) })
                      }
                    />
                    <Slider
                      style={{ flex: 1 }}
                      color="grape"
                      value={cmykValues.m}
                      onChange={(v) => setCmykValues({ ...cmykValues, m: v })}
                      label={null}
                    />
                  </Group>
                  <Group gap="xs" align="center">
                    <NumberInput
                      ...
                      value={cmykValues.y}
                      onChange={(v) =>
                        setCmykValues({ ...cmykValues, y: Number(v) })
                      }
                    />
                    <Slider
                      style={{ flex: 1 }}
                      color="yellow.4"
                      value={cmykValues.y}
                      onChange={(v) => setCmykValues({ ...cmykValues, y: v })}
                      label={null}
                    />
                  </Group>
                  <Group gap="xs" align="center">
                    <NumberInput
                      ...
                      value={cmykValues.k}
                      onChange={(v) =>
                        setCmykValues({ ...cmykValues, k: Number(v) })
                      }
                    />
                    <Slider
                      style={{ flex: 1 }}
                      color="black"
                      value={cmykValues.k}
                      onChange={(v) => setCmykValues({ ...cmykValues, k: v })}
                      label={null}
                    />
                  </Group>
                </SimpleGrid>
              ...
              <Stack gap="xs">
                ...
                <Group grow>
                  ...
                  <Select
                    ...
                    data={allColors}
                    value={mixSecondary}
                    onChange={(v) => setMixSecondary(v || "white")}
                    searchable
                  />
                </Group>
                <Stack gap={0}>
                  <Group justify="space-between">
                    <Text size="xs">
                      Mix Amount: <b>{mixPercent}%</b> {mixBase}
                    </Text>
                    <NumberInput
                      value={mixPercent}
                      onChange={setMixPercent}
                      min={0}
                      max={100}
                      step={5}
                      size="xs"
            ...
            {defMode === "snippets" && (
              <Stack gap="xs">
                ...
                <SimpleGrid cols={2}>
                  <Select
                    label={snipType === "fcolorbox" ? "Fill Color" : "Color"}
                    data={allColors}
                    value={snipColor}
                    onChange={(v) => setSnipColor(v || "blue")}
                    searchable
                  />
                  {snipType === "fcolorbox" && (
                    <Select
                      ...
                      data={allColors}
                      value={snipSecColor}
                      onChange={(v) => setSnipSecColor(v || "black")}
                      searchable
                    />
                  )}
                </SimpleGrid>
                ...
                {["textcolor", "colorbox", "fcolorbox"].includes(snipType) && (
                  <TextInput
                    label="Content Text"
                    value={snipText}
                    onChange={(e) => setSnipText(e.currentTarget.value)}
                  />
                )}

                <Paper p="xs" bg="var(--mantine-color-default-hover)" mt="xs">
                  <Code block>
                    {snipType === "textcolor" &&
                      `\\textcolor{${snipColor}}{${snipText}}`}
                    {snipType === "colorbox" &&
                      `\\colorbox{${snipColor}}{${snipText}}`}
                    {snipType === "fcolorbox" &&
                      `\\fcolorbox{${snipSecColor}}{${snipColor}}{${snipText}}`}
                ...
                <Group grow>
                  ...
                  {onInsert && (
                    <Button onClick={handleInsertSnippet} color="teal">
                      Insert
                    </Button>
                  )}
                </Group>
        ...
        <Stack gap="xs" style={{ overflowY: "auto" }}>
          {customColors.map((c) => (
            <Group
              ...
              style={{
                borderRadius: 4,
                borderLeft: `4px solid ${c.previewHex}`,
              }}
            >
              <Group gap="sm">
                {/* Color Preview Block */}
                ...
                <Stack gap={0}>
                  <Group gap={6}>
                    <FontAwesomeIcon
                      icon={faCheckCircle}
                      style={{ width: 10, height: 10, color: "#40c057" }}
                    />
                    <Text size="sm" fw={700}>
                      {c.name}
                    </Text>
                  ...
                  <Code style={{ fontSize: 10, padding: 2 }}>
                    {c.model === "named (mix)"
